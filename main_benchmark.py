...
Shows performance of CBO (Bandit/Frozen) vs Baselines (IDSelector, PostFilter, Bitmap BF).
Also tracks overheads (predictor & routing).
"""
...
def main():
    out_dir = config.RESULTS_DIR / f"main_freeze_200k_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    cache = {
        ...
        "ref_ids": [[None]*NUM_QUERIES for _ in range(len(filters))]
    }
    ...
    for f_idx, fspec in enumerate(filters):
        ...
        cbo_entries = [d for d in all_query_details if d.phase == "frozen" and d.filter_idx == f_idx]
    ...
    csv_path = out_dir / "phase_1_selectivity_frozen.csv"
